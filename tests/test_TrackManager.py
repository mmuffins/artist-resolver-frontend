...
async def test_parse_artist_json_with_nested_objects():
    ...
    for i in range(len(expected)):
        ...
        assert actual_artist.mbid == expected_artist["id"], f"MBID mismatch at index {i}: expected {expected_artist["id"]}, got {actual_artist.mbid}"
        assert actual_artist.name == expected_artist["name"], f"name mismatch at index {i}: expected {expected_artist["name"]}, got {actual_artist.name}"
        assert actual_artist.sort_name == expected_artist["sort_name"], f"sort_name mismatch at index {i}: expected {expected_artist["sort_name"]}, got {actual_artist.sort_name}"
        assert actual_artist.type == expected_artist["type"], f"type mismatch at index {i}: expected {expected_artist["type"]}, got {actual_artist.type}"
